...
description: 仅在 vLLM Ascend（昇腾）推理出现精度或输出质量问题时使用：模型输出乱码、复读、答案错误、与参考实现/其他后端结果不一致、基准评测分数不达标，需要"复现→隔离→根因→修复→回归"的系统性精度排查闭环。用户的请求是普通开发调试时不要使用——如"看下代码报错"、"调试一下"、"服务起不来"、"环境配置问题"、"改个功能"，这些是常规开发任务不是精度问题；也不用于纯性能调优（吞吐/时延优化）。
---
