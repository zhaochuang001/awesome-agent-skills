...
python scripts/migrate.py \
  ...
  [--sync-weights]             # 目标机权重缺失/不完整时自动从源机同步
...
给了 `--weights-path` 时，在起容器**之前**检查目标机上的权重：存在且与源机大小一致（容差 1%）才继续，否则返回 `needs_input` 列出明细——服务起不来最常见的坑就是目标机没权重，提前止损。加 `--sync-weights` 则自动从源机同步缺失/不完整的权重（rsync 断点续传，服务器间直传），同步后复查。`--plan` 干跑也会真实检查权重（只读）并列出状态。
